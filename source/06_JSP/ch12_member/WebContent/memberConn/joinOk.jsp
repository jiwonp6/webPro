...
<body>
	...
	MemberDao_connPool mDao = new MemberDao_connPool();	//싱글톤 아님
	int result = mDao.confirmId(dto.getId());	//ID 중복체크
	if(result == MemberDao_connPool.MEMBER_NONEXISTENT){	//사용가능한 id
		result = mDao.joinMember(dto);	//회원가입
		if(result == MemberDao_connPool.SUCCESS){	//회원가입성공
