-- EX1. 사용자에게 부서번호 입력받아 부서 정보 뿌리기
SELECT * FROM DEPT WHERE DEPTNO=30;
INSERT INTO DEPT VALUES (50, 'IT', 'SEOUL');
SELECT * FROM DEPT; --COMMIT을 안하면 없다고 나온다.
ROLLBACK;
-- EX2. 
SELECT * FROM DEPT WHERE DEPTNO=30; -- java에서 그대로 실행가능
    --해당부서번호 사원의 정보 (사번, 이름, 급여, 상사명)
    SELECT W.EMPNO, W.ENAME WORKER, W.SAL, M.ENAME MANAGER
        FROM EMP W, EMP M
        WHERE W.MGR = M.EMPNO AND W.DEPTNO = 30;
-- EX3. 사용자에게 부서명을 입력받아 부서 정보 뿌리기
SELECT * FROM DEPT WHERE DNAME = UPPER('sales');
    --사번, 이름, 급여, 급여등급
SELECT EMPNO, ENAME, SAL, GRADE
    FROM EMP E, SALGRADE, DEPT D
    WHERE E.SAL BETWEEN LOSAL AND HISAL AND E.DEPTNO = D.DEPTNO AND DNAME = UPPER('SALES');
SELECT EMPNO, ENAME, SAL, GRADE
    FROM EMP, SALGRADE
    WHERE SAL BETWEEN LOSAL AND HISAL AND DEPTNO = (SELECT DEPTNO FROM DEPT WHERE DNAME = UPPER('SALES'));
    
-- EX4.     
    
