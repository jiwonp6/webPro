...
<head>
	...
	<script>
		function trClicked(bid){
			location.href = '${conPath}/mvcboard/detail.do?bid='+bid+'&pageNum=${paging.currentPage}';
		}
	</script>
</head>
<body>
	...
	<c:set var="ordNum" value="${paging.orderNum }"/>
	<c:set var="invNum" value="${paging.inverseNum }"/>
	<table>
		<tr>
	...
	<div class="paging">
	  	<c:if test="${paging.startPage > paging.blockSize }">
	  		[ <a href="${conPath }/mvcboard/boardList.do?pageNum=${paging.startPage-1 }"> 이 전 </a>]
	  	</c:if>
	  	<c:forEach var="i" begin="${paging.startPage }" end="${paging.endPage }">
	  		...
	  		<c:if test="${i != paging.currentPage }">
	  			[ <a href="${conPath }/mvcboard/boardList.do?pageNum=${i }">${i }</a> ]
	  		</c:if>
	  	</c:forEach>
	  	<c:if test="${paging.endPage < paging.pageCnt }">
	  		[ <a href="${conPath }/mvcboard/boardList.do?pageNum=${paging.endPage+1 }"> 다 음 </a>]
	  	</c:if>
  	</div>
</body>
</html>